...
function send(){
	...
	if (form_name.name.value.match(/\s/g)) {
        alert("성명에 공백이 존재합니다.");
        form_name.name.focus();
        return;
    }
	
	//id  5~16자 이내의 영문이나 숫자만 가능합니다.  
    if (!form_name.userid.value.match(/^[a-zA-z0-9]{5,16}$/g)) {
        alert("아이디는 영문 대소문자와 숫자 5~16자리로 입력해야합니다!");
        form_name.userid.focus();
        return;
    }
    
	//비밀번호  6~12자 이내로 가능합니다.   
	 if (form_name.passwd.value.length<6 || form_name.passwd.value.length>12) {
        alert("비밀번호는 6~12자리로 입력해야합니다!");
        form_name.passwd.focus();
        return;
    }
	
	//비밀번호확인 * 비밀번호 확인을 위해서 비밀번호를 한번 더 입력해주세요.  
	if (form_name.repasswd.value =="") {
        alert("비밀번호를 다시 입력하세요.");
        form_name.repasswd.focus();
        return;
    }
	
	if (!(form_name.repasswd.value == form_name.repasswd.value)) {
	...
	// 주소구분 
	for(x=0, flag =0; x<form_name.gubun.length; x++){
	...
	//관심분야  
	for(x=0, flag =0; x<form_name.fa.length; x++){
	...
	form_name.action = "userinfo_bean.jsp";
	alert("전송 성공");
	form_name.submit(); 
}
function reset(){
            ...
            <table cellpadding=0 cellspacing=0 border=0 width=100%>
              ...
              <tr bgcolor=#ffffff>
                <td colspan=3 align=center>
                 <a href ="javascript:send()"><img src="./img/u_bt06.gif" vspace=3 border=0 name=img3>
                 <a href ="javascript:reset()"> <img src="./img/u_bt05.gif" border=0 hspace=10 vspace=3 name=img4>
                </td>
              </tr>
            </table> 
          </td>
