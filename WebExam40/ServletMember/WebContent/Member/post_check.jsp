...
<%
	...
	List<Zipcode> list = dao.zipcodeList(addr);
%>
<HTML>
<HEAD>
<TITLE>우편번호 찾기</TITLE>
...
<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 WIDTH=330>
  <TR BGCOLOR=#7AAAD5>
    <td align=left><img src="Model/img/u_b02.gif"></td>
    <td align=center><FONT COLOR="#FFFFFF"><b>우편번호 찾기</FONT></td>
    <td align=right><img src= "Model/img/u_b03.gif"></td>
  </tr>
</table>
    ...
    <FORM NAME="inquiry" METHOD="post" ACTION="post_check.jsp" onSubmit="return check();">
      <TR>
        <TD ALIGN=CENTER><br>
          ...
          <INPUT TYPE="image" src="Model/img/u_bt08.gif" hspace=10>
        </TD>
      </TR>
      <TR>
        <TD ALIGN=CENTER>
...
%>
      <TR>
        <TD>
        <a href = "javascript:sendAddr('<%= list.get(x).getZipcode() %>','<%= list.get(x).getSido() %>','<%= list.get(x).getGugun() %>','<%= list.get(x).getDong() %>','<%= list.get(x).getBunji() %>')">
        <%= list.get(x).getZipcode() %><%= list.get(x).getSido() %><%= list.get(x).getGugun() %><%= list.get(x).getDong() %><%= list.get(x).getBunji() %>
        </a>
        </TD>
      </TR>
<%
...
<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 WIDTH=330>
  <TR BGCOLOR=#7AAAD5>
    <td align=left><img src="Model/img/u_b04.gif"></td>
    <td align=right><img src="Model/img/u_b05.gif"></td>
  </tr>
  <tr>
    <td colspan=2 align=center>
      <img src="Model/img/u_bt13.gif" border=0 vspace=5>    </td>
  </tr>
</table>
</BODY>
</HTML>
