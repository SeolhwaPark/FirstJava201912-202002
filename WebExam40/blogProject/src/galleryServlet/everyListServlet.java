...
public class everyListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int totcount = dao.everyCount(s_query); // 총 게시글수

		// 페이지계산 기본방법
		int nowpage = 1;// 현재페이지
		int maxlist = 3; // 페이지당 글 수
		...
		List<EveryVO> list = null;
		list = dao.everyList();
		if (key.equals("")) {
			list = dao.everyList(pagestart, endpage);
		} else
			list = dao.everyList(s_query, pagestart, endpage);

		String pageSkip = PageIndex.pageList(nowpage, totpage, "every_list", addtag);
		...
		RequestDispatcher dis = request.getRequestDispatcher("gallery/every_list.jsp");
