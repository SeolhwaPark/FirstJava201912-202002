...
@WebServlet("/ui_list")
public class uiListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public uiListServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int totcount = dao.uiCount(s_query); // 총 게시글수
		...
		list = dao.uiList();
		if(key.equals("")) {
			list = dao.uiList(pagestart, endpage);
		} else
			list = dao.uiList(s_query, pagestart, endpage);
		
		String pageSkip = PageIndex.pageList(nowpage, totpage, "ui_list", addtag);
		...
		RequestDispatcher dis = request.getRequestDispatcher("study/ui_list.jsp");
